...
/* 				M.Bedirhan ÇAĞLAR ******** 06.04.2016 						 */

/* Bu bir satranç oyunudur sadece taşların hareketleri kontrol edilmektedir */
...
int bosluk1(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);/*Bu fonksiyonlar filin */
int bosluk2(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);/*durumuna göre boslukları */
int bosluk3(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);/* konrol eder */ 
int bosluk4(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);
int kale_bosluk1(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);/*bu fonksiyonlar ise kale için boşluk kontrolü yapar. */
int kale_bosluk2(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);
int kale_bosluk3(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);
int kale_bosluk4(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow);
...
int isPieceMovable(char *board)/* tasa göre fonksiyon cagirip uygun olup */ 
{								/* olmamasını kontrol eder. */
	char sourceCol;
	int sourceRow;
	char targetCol;
	int targetRow;
...
/* ****BU FONKSIYONLAR TASLARIN UYGUNLUĞUNU OYNAYIP OYNAMAMA DURUMUNU RETURN 
	EDER **** */
int isPawnMovable(char *board,char sourceCol,int sourceRow,char targetCol,
int targetRow)
{
...
(sourceCol,sourceRow)]=='q' )	/* q ve r kontrolü ilerde vezir fonksiyonumda 
...
targetRow)]<=122))
		...
		else if(abs(sourceCol-targetCol)==1 && targetRow==sourceRow && !((int)board[
find_stone(targetCol,targetRow)]>=97 && (int)board[find_stone(targetCol,
targetRow)]<=122))
		return 1;

		else if(sourceCol==targetCol	&& targetRow-sourceRow>0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && 
(int)board[find_stone(targetCol,targetRow)]<=122) )
		return kale_bosluk1(board,sourceCol,sourceRow,targetCol,targetRow);
			
		else if(sourceCol==targetCol	&& targetRow-sourceRow<0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && 
(int)board[find_stone(targetCol,targetRow)]<=122) )
		return kale_bosluk2(board,sourceCol,sourceRow,targetCol,targetRow);
		
		else if((int)targetCol-(int)sourceCol<0 && targetRow==sourceRow && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && 
(int)board[find_stone(targetCol,targetRow)]<=122))
		return kale_bosluk3(board,sourceCol,sourceRow,targetCol,targetRow);
		
		else if((int)targetCol-(int)sourceCol>0 && targetRow==sourceRow && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && 
(int)board[find_stone(targetCol,targetRow)]<=122) )
		return kale_bosluk4(board,sourceCol,sourceRow,targetCol,targetRow);
...
!((int)board[find_stone(targetCol,targetRow)]>=65 && 
(int)board[find_stone(targetCol,targetRow)]<=90) )
		return kale_bosluk1(board,sourceCol,sourceRow,targetCol,targetRow);
...
int targetRow)
{
	if(board[find_stone(sourceCol,sourceRow)]=='b' || board[find_stone
(sourceCol,sourceRow)]=='q') /* q ve r kontrolü ilerde vezir fonksiyonumda fili çağırdığım icindir */
	{
...
(targetCol,targetRow)]<=122))
		...
		else if(targetRow-sourceRow>0 && (int)targetCol-(int)sourceCol>0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && (int)board
[find_stone(targetCol,targetRow)]<=122 ))
			return bosluk1(board,sourceCol,sourceRow,targetCol,targetRow);

		else if(targetRow-sourceRow<0 && (int)targetCol-(int)sourceCol<0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && (int)board
[find_stone(targetCol,targetRow)]<=122 ))
			return bosluk2(board,sourceCol,sourceRow,targetCol,targetRow);

		else if(targetRow-sourceRow>0 && (int)targetCol-(int)sourceCol<0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && (int)board
[find_stone(targetCol,targetRow)]<=122 ))
			return bosluk3(board,sourceCol,sourceRow,targetCol,targetRow);

		else if(targetRow-sourceRow<0 && (int)targetCol-(int)sourceCol>0 && 
!((int)board[find_stone(targetCol,targetRow)]>=97 && (int)board
[find_stone(targetCol,targetRow)]<=122 ))
			return bosluk4(board,sourceCol,sourceRow,targetCol,targetRow);
		else return 0;
	}
	else{
...
!((int)board[find_stone(targetCol,targetRow)]>=65 && (int)board
[find_stone(targetCol,targetRow)]<=90 ))
			return bosluk1(board,sourceCol,sourceRow,targetCol,targetRow);
...
int targetRow)
{
	...
	else if(abs(sourceCol-targetCol)==abs(sourceRow-targetRow))
		return isBishopMovable(board,sourceCol,sourceRow,targetCol,targetRow);
	else return 0;
}
int bosluk1(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
		if(board[find_stone((char)((int)targetCol-i),targetRow-i)]!=' ')
			return 0;
	}
	return 1;
}
int bosluk2(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
...
int bosluk3(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
...
int bosluk4(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
...
int kale_bosluk1(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
		if(board[find_stone(targetCol,targetRow-i)]!=' ')
			return 0;
	}
	return 1;
}
int kale_bosluk2(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetRow-sourceRow; i++)
	{
...
int kale_bosluk3(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetCol-sourceCol; i++)
	{
		if(board[find_stone((char)((int)targetCol+i),targetRow)]!=' ')
			return 0;
	}
	return 1;
}
int kale_bosluk4(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow)
{
	int i;
	for(i=1; i<targetCol-sourceCol; i++)
	{
